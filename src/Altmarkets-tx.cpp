...
static bool AppInitRawTx(int argc, char* argv[])
{
    ...
    if (argc<2 || mapArgs.count("-?") || mapArgs.count("-h") || mapArgs.count("-help"))
    {
        ...
        std::string strUsage = _("Altmarkets Altmarkets-tx utility version") + " " + FormatFullVersion() + "\n\n" +
            _("Usage:") + "\n" +
              "  Altmarkets-tx [options] <hex-tx> [commands]  " + _("Update hex-encoded Altmarkets transaction") + "\n" +
              "  Altmarkets-tx [options] -create [commands]   " + _("Create hex-encoded Altmarkets transaction") + "\n" +
              "\n";
...
static void MutateTxAddOutAddr(CMutableTransaction& tx, const string& strInput)
{
    ...
    CAltmarketsAddress addr(strAddr);
    if (!addr.IsValid())
...
{
    if (!value.isNull() && !value.isStr())
        throw runtime_error("Amount is not a number of string");
    CAmount amount;
...
static void MutateTxSign(CMutableTransaction& tx, const string& flagStr)
{
    ...
    UniValue prevtxsObj = registers["prevtxs"];
    {
        for (unsigned int previdx = 0; previdx < prevtxsObj.size(); previdx++) {
            ...
            map<string,UniValue::VType> types = {{"txid", UniValue::VSTR}, {"vout",UniValue::VNUM}, {"scriptPubKey",UniValue::VSTR}};
...
static int CommandLineRawTx(int argc, char* argv[])
{
    ...
    try {
        ...
        if (!fCreateBlank) {
            ...
            // param: hex-encoded Altmarkets transaction
