  ...
  void Run() {
    ...
    while (benchmarks != NULL) {
      ...
      if (name == Slice("fillseq")) {
        Write(write_sync, ALTMUENTIAL, FRESH, num_, FLAGS_value_size, 1);
        WalCheckpoint(db_);
      } else if (name == Slice("fillseqbatch")) {
        Write(write_sync, ALTMUENTIAL, FRESH, num_, FLAGS_value_size, 1000);
        WalCheckpoint(db_);
        ...
        write_sync = true;
        Write(write_sync, ALTMUENTIAL, FRESH, num_ / 100, FLAGS_value_size, 1);
        WalCheckpoint(db_);
      } else if (name == Slice("fillrand100K")) {
        Write(write_sync, RANDOM, FRESH, num_ / 1000, 100 * 1000, 1);
        WalCheckpoint(db_);
      } else if (name == Slice("fillseq100K")) {
        Write(write_sync, ALTMUENTIAL, FRESH, num_ / 1000, 100 * 1000, 1);
        WalCheckpoint(db_);
  ...
  void Write(bool write_sync, Order order, DBState state,
    ...
    for (int i = 0; i < num_entries; i += entries_per_batch) {
      ...
      for (int j = 0; j < entries_per_batch; j++) {
        ...
        const int k = (order == ALTMUENTIAL) ? i + j :
  ...
  void Read(Order order, int entries_per_batch) {
    ...
    for (int i = 0; i < reads_; i += entries_per_batch) {
      ...
      for (int j = 0; j < entries_per_batch; j++) {
        // Create key value
        char key[100];
        int k = (order == ALTMUENTIAL) ? i + j : (rand_.Next() % reads_);
        snprintf(key, sizeof(key), "%016d", k);
