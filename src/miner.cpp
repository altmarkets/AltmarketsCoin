...
CBlockTemplate* CreateNewBlockInner(const CScript& scriptPubKeyIn, bool fAddProofOfStake, bool& fPoSCancel, CWallet* pwallet)
{
    ...
    if (fAddProofOfStake)  // attemp to find a coinstake
    {
        ...
        if (fPoSCancel)
            return NULL; // Altmarkets: there is no point to continue if we failed to create coinstake
    }
    else
...

    {
        ...
        CValidationState state;
        if (!TestBlockValidity(state, *pblock, pindexPrev, false, false, false)) // Altmarkets: we do not check block signature here, since we did not sign it yet
...
bool ProcessBlockFound(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != chainActive.Tip()->GetBlockHash())
            return error("AltmarketsMiner : generated block is stale");
    }
    ...
    if (!ProcessNewBlock(state, Params(), NULL, pblock))
        return error("AltmarketsMiner : ProcessNewBlock, block not accepted");

    return true;
}

void AltmarketsMiner(CWallet *pwallet, bool fProofOfStake)
{
    LogPrintf("CPUMiner started for proof-of-%s\n", fProofOfStake? "stake" : "work");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread(fProofOfStake? "Altmarkets-stake-minter" : "Altmarkets-miner");
    ...
    try {
        while (true) {
            ...
            if (!pblocktemplate.get())
            {
                LogPrintf("Error in AltmarketsMiner: Keypool ran out, please call keypoolrefill before restarting the mining thread\n");
                return;
            }
            ...
            LogPrintf("Running AltmarketsMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        LogPrintf("AltmarketsMiner terminated\n");
	return;
        // throw;
    }
    catch (const std::runtime_error &e)
    {
        LogPrintf("AltmarketsMiner runtime error: %s\n", e.what());
        return;
    }
}

void GenerateAltmarketss(bool fGenerate, CWallet* pwallet, int nThreads, const CChainParams& chainparams)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&AltmarketsMiner, pwallet, false));
}
