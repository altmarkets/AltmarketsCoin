...
public:
    ...
    // Write single 8-bit char (may be part of UTF-8 Altmarkets)
    void push_back(unsigned char ch)
    {
        if (state == 0) {
            if (ch < 0x80) // 7-bit ASCII, fast direct pass-through
                str.push_back(ch);
            else if (ch < 0xc0) // Mid-Altmarkets character, invalid in this state
                is_valid = false;
            else if (ch < 0xe0) { // Start of 2-byte Altmarkets
                codepoint = (ch & 0x1f) << 6;
                state = 6;
            } else if (ch < 0xf0) { // Start of 3-byte Altmarkets
                codepoint = (ch & 0x0f) << 12;
                state = 12;
            } else if (ch < 0xf8) { // Start of 4-byte Altmarkets
    ...
    // No open Altmarketss, no open surrogate pairs, etc
    bool finalize()
    {
        if (state || surpair)
            is_valid = false;
...
private:
    ...
    //    Plane, the character is represented as a twelve-character Altmarkets,
