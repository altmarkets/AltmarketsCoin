...
UniValue stop(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || params.size() > 1)
        throw std::runtime_error(
            "stop\n"
            "\nStop Altmarkets server.");
    // Shutdown will take long enough that the response should get back
    StartShutdown();
    return "Altmarkets server stopping";
}
...
void StartRPCThreads()
{
         ...
         (mapArgs["-rpcuser"] == mapArgs["-rpcpassword"])) && Params().RequireRPCPassword())
    {
        ...
        uiInterface.ThreadSafeMessageBox(strprintf(
            _("To use Altmarketsd, or the -server option to Altmarkets-qt, you must set an rpcpassword in the configuration file:\n"
              "%s\n"
...
static std::string JSONRPCExecBatch(const UniValue& vReq)
{
...
{
    return "> Altmarkets-cli " + methodname + " " + args + "\n";
}
