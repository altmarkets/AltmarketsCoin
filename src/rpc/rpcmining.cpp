...
UniValue getgenerate(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 0)
        throw std::runtime_error(
            ...
            "It is set with the command line argument -gen (or Altmarkets.conf setting gen)\n"
...
UniValue setgenerate(const UniValue& params, bool fHelp)
{
    ...
    else // Not -regtest: start generate thread, return immediately
    {
        ...
        GenerateAltmarketss(fGenerate, pwalletMain, nGenProcLimit, Params());
    }
...
UniValue getstakinginfo(const UniValue& params, bool fHelp)
{
    ...
    uint64_t nExpectedTime = staking ? (Params().GetConsensus().nPoSTargetSpacing * nNetworkWeight / nWeight) : -1;
...
// NOTE: Unlike wallet RPC (which use Altmarkets values), mining RPCs follow GBT (BIP 22) in using satoshi amounts
...
UniValue getwork(const UniValue& params, bool fHelp)
{
    ...
    if (vNodes.empty())
        throw JSONRPCError(RPC_CLIENT_NOT_CONNECTED, "Altmarkets is not connected!");

    if (IsInitialBlockDownload())
        throw JSONRPCError(RPC_CLIENT_IN_INITIAL_DOWNLOAD, "Altmarkets is downloading blocks...");
    ...
    else
    {
        ...
        tx.vin[0].scriptSig = mapNewBlock[pdata->hashMerkleRoot].second;
        pblock->vtx[0] = tx;
...
UniValue getblocktemplate(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() > 1)
        throw std::runtime_error(
            ...
            "See https://en.Altmarkets.it/wiki/BIP_0022 for full specification.\n"
...
{ //  category              name                      actor (function)         okSafeMode threadSafe reqWallet
  //  --------------------- ------------------------  -----------------------  ---------- ---------- ---------
    ...
    { "mining",             "getsubsidy",             &getsubsidy,             true,      true,       false },
