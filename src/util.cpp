...
#else
    const char* pszModule = "Altmarkets";
#endif
    if (pex)
        return strprintf(
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\Altmarkets
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\Altmarkets
    // Mac: ~/Library/Application Support/Altmarkets
    // Unix: ~/.Altmarkets
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Altmarkets";
#else
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "Altmarkets.conf"));
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()){
        // Create Altmarkets.conf if it does not exist
        FILE* configFile = fopen(GetConfigFile().string().c_str(), "a");
        if (configFile != NULL) {
            // Write Altmarkets.conf file with random username and password.
            WriteConfigFile(configFile);
            // New Altmarkets.conf file written, now read it.
        }
    }
    ...
    {
        // Don't overwrite existing settings so command line settings override Altmarkets.conf
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "Altmarketsd.pid"));
