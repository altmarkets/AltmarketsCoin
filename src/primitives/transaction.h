...
#ifndef Altmarkets_PRIMITIVES_TRANSACTION_H
#define Altmarkets_PRIMITIVES_TRANSACTION_H
...
public:
    COutPoint prevout;
    CScript scriptSig;
    uint32_t nAltmarkets;

    CTxIn()
    {
        nAltmarkets = std::numeric_limits<unsigned int>::max();
    }

    explicit CTxIn(COutPoint prevoutIn, CScript scriptSigIn=CScript(), uint32_t nAltmarketsIn=std::numeric_limits<unsigned int>::max());
    CTxIn(uint256 hashPrevTx, uint32_t nOut, CScript scriptSigIn=CScript(), uint32_t nAltmarketsIn=std::numeric_limits<uint32_t>::max());
    ...
    inline void SerializationOp(Stream& s, Operation ser_action, int nType, int nVersion) {
        READWRITE(prevout);
        READWRITE(scriptSig);
        READWRITE(nAltmarkets);
    }

    bool IsFinal() const
    {
        return (nAltmarkets == std::numeric_limits<uint32_t>::max());
    }
...
#endif // Altmarkets_PRIMITIVES_TRANSACTION_H
