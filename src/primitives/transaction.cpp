...
CTxIn::CTxIn(COutPoint prevoutIn, CScript scriptSigIn, uint32_t nAltmarketsIn)
{
    prevout = prevoutIn;
    scriptSig = scriptSigIn;
    nAltmarkets = nAltmarketsIn;
}

CTxIn::CTxIn(uint256 hashPrevTx, uint32_t nOut, CScript scriptSigIn, uint32_t nAltmarketsIn)
{
...
std::string CTxIn::ToString() const
{
    ...
    if (nAltmarkets != std::numeric_limits<unsigned int>::max())
        str += strprintf(", nAltmarkets=%u", nAltmarkets);
    str += ")";
    return str;
}
