...
UniValue getnewaddress(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() > 1)
        throw std::runtime_error(
            ...
            "\nReturns a new Altmarkets address for receiving payments.\n"
            "If 'account' is specified (recommended), it is added to the address book \n"
            "so payments received with the address will be credited to 'account'.\n"
            "\nArguments:\n"
            "1. \"account\"        (string, optional) The account name for the address to be linked to. if not provided, the default account \"\" is used. It can also be set to the empty string \"\" to represent the default account. The account does not need to exist, it will be created if there is no account by the given name.\n"
            "\nResult:\n"
            "\"Altmarketsaddress\"    (string) The new Altmarkets address\n"
            "\nExamples:\n"
    ...
    return CAltmarketsAddress(keyID).ToString();
}


CAltmarketsAddress GetAccountAddress(std::string strAccount, bool bForceNew=false)
{
    ...
    return CAltmarketsAddress(account.vchPubKey.GetID());
}
...
UniValue getaccountaddress(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw std::runtime_error(
            ...
            "\nReturns the current Altmarkets address for receiving payments to this account.\n"
            "\nArguments:\n"
            "1. \"account\"       (string, required) The account name for the address. It can also be set to the empty string \"\" to represent the default account. The account does not need to exist, it will be created and a new address created  if there is no account by the given name.\n"
            "\nResult:\n"
            "\"Altmarketsaddress\"   (string) The account Altmarkets address\n"
            "\nExamples:\n"
...
UniValue getrawchangeaddress(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() > 1)
        throw std::runtime_error(
            "getrawchangeaddress\n"
            "\nReturns a new Altmarkets address, for receiving change.\n"
...
UniValue setaccount(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 1 || params.size() > 2)
        throw std::runtime_error(
            "setaccount \"Altmarketsaddress\" \"account\"\n"
            "\nSets the account associated with the given address.\n"
            "\nArguments:\n"
            "1. \"Altmarketsaddress\"  (string, required) The Altmarkets address to be associated with an account.\n"
    ...
    CAltmarketsAddress address(params[0].get_str());
    if (!address.IsValid())
        throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid Altmarkets address");
...
UniValue getaccount(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw std::runtime_error(
            "getaccount \"Altmarketsaddress\"\n"
            "\nReturns the account associated with the given address.\n"
            "\nArguments:\n"
            "1. \"Altmarketsaddress\"  (string, required) The Altmarkets address for account lookup.\n"
            "\nResult:\n"
...
UniValue getaddressesbyaccount(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw std::runtime_error(
            ...
            "  \"Altmarketsaddress\"  (string) a Altmarkets address associated with the given account\n"
            "  ,...\n"
            "]\n"
            "\nExamples:\n"
    ...
    for(const std::pair<CAltmarketsAddress, CAddressBookData>& item : pwalletMain->mapAddressBook)
    {
        const CAltmarketsAddress& address = item.first;
...
UniValue sendtoaddress(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 4)
        throw std::runtime_error(
            "sendtoaddress \"Altmarketsaddress\" amount ( \"comment\" \"comment-to\" )\n"
            "\nSend an amount to a given address. The amount is a real and is rounded to the nearest 0.00000001\n"
            + HelpRequiringPassphrase() +
            "\nArguments:\n"
            "1. \"Altmarketsaddress\"  (string, required) The Altmarkets address to send to.\n"
            "2. \"amount\"      (numeric, required) The amount in Altmarkets to send. eg 0.1\n"
...
UniValue listaddressgroupings(const UniValue& params, bool fHelp)
{
    if (fHelp)
        throw std::runtime_error(
            ...
            "  [\n"
            "    [\n"
            "      \"Altmarketsaddress\",     (string) The Altmarkets address\n"
            "      amount,                 (numeric) The amount in Altmarkets\n"
    ...
    for(std::set<CTxDestination> grouping : pwalletMain->GetAddressGroupings())
    {
        ...
        for(CTxDestination address : grouping)
        {
            ...
            addressInfo.push_back(CAltmarketsAddress(address).ToString());
            addressInfo.push_back(ValueFromAmount(balances[address]));
            {
                LOCK(pwalletMain->cs_wallet);
                if (pwalletMain->mapAddressBook.find(CAltmarketsAddress(address).Get()) != pwalletMain->mapAddressBook.end())
                    addressInfo.push_back(pwalletMain->mapAddressBook.find(CAltmarketsAddress(address).Get())->second.name);
            }
...
UniValue signmessage(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 2)
        throw std::runtime_error(
            "signmessage \"Altmarketsaddress\" \"message\"\n"
            "\nSign a message with the private key of an address"
            + HelpRequiringPassphrase() + "\n"
            "\nArguments:\n"
            "1. \"Altmarketsaddress\"  (string, required) The Altmarkets address to use for the private key.\n"
    ...
    CAltmarketsAddress addr(strAddress);
    if (!addr.IsValid())
...
UniValue getreceivedbyaddress(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 1 || params.size() > 2)
        throw std::runtime_error(
            "getreceivedbyaddress \"Altmarketsaddress\" ( minconf )\n"
            "\nReturns the total amount received by the given Altmarketsaddress in transactions with at least minconf confirmations.\n"
            "\nArguments:\n"
            "1. \"Altmarketsaddress\"  (string, required) The Altmarkets address for transactions.\n"
            "2. minconf             (numeric, optional, default=1) Only include transactions confirmed at least this many times.\n"
            "\nResult:\n"
            "amount   (numeric) The total amount in Altmarkets received at this address.\n"
            "\nExamples:\n"
            "\nThe amount from transactions with at least 1 confirmation\n"
            + HelpExampleCli("getreceivedbyaddress", "\"ST5ZrZNe3JUo7ZycKEYQQiQAWd9y54F4XZ\"") +
            "\nThe amount including unconfirmed transactions, zero confirmations\n"
            + HelpExampleCli("getreceivedbyaddress", "\"ST5ZrZNe3JUo7ZycKEYQQiQAWd9y54F4XZ\" 0") +
            "\nThe amount with at least 10 confirmation, very safe\n"
            + HelpExampleCli("getreceivedbyaddress", "\"ST5ZrZNe3JUo7ZycKEYQQiQAWd9y54F4XZ\" 10") +
            "\nAs a json rpc call\n"
            + HelpExampleRpc("getreceivedbyaddress", "\"ST5ZrZNe3JUo7ZycKEYQQiQAWd9y54F4XZ\", 10")
       );

    // Altmarkets address
    CAltmarketsAddress address = CAltmarketsAddress(params[0].get_str());
    if (!address.IsValid())
        throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid Altmarkets address");
...
UniValue getreceivedbyaccount(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 1 || params.size() > 2)
        throw std::runtime_error(
            ...
            "2. minconf          (numeric, optional, default=1) Only include transactions confirmed at least this many times.\n"
            "\nResult:\n"
            "amount              (numeric) The total amount in Altmarkets received for this account.\n"
            "\nExamples:\n"
...
UniValue getbalance(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() > 3)
        throw std::runtime_error(
            ...
            "\nThe total amount in the server across all accounts, with at least 9 confirmations\n"
...
UniValue movecmd(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 3 || params.size() > 5)
        throw std::runtime_error(
            ...
            "\nExamples:\n"
            "\nMove 0.01 Altmarkets from the default account to the account named tabby\n"
            + HelpExampleCli("move", "\"\" \"tabby\" 0.01") +
            "\nMove 0.01 Altmarkets timotei to akiko with a comment and funds have 10 confirmations\n"
...
UniValue sendfrom(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 3 || params.size() > 6)
        throw std::runtime_error(
            "sendfrom \"fromaccount\" \"toAltmarketsaddress\" amount ( minconf \"comment\" \"comment-to\" )\n"
            "\nSent an amount from an account to a Altmarkets address.\n"
            "The amount is a real and is rounded to the nearest 0.00000001."
            + HelpRequiringPassphrase() + "\n"
            "\nArguments:\n"
            "1. \"fromaccount\"       (string, required) The name of the account to send funds from. May be the default account using \"\".\n"
            "2. \"toAltmarketsaddress\"  (string, required) The Altmarkets address to send funds to.\n"
            "3. amount                (numeric, required) The amount in Altmarkets. (transaction fee is added on top).\n"
            ...
            "\"transactionid\"        (string) The transaction id.\n"
            "\nExamples:\n"
            "\nSend 0.01 Altmarkets from the default account to the address, must have at least 1 confirmation\n"
            + HelpExampleCli("sendfrom", "\"\" \"ST5ZrZNe3JUo7ZycKEYQQiQAWd9y54F4XZ\" 0.01") +
            "\nSend 0.01 from the tabby account to the given address, funds must have at least 10 confirmations\n"
            + HelpExampleCli("sendfrom", "\"tabby\" \"ST5ZrZNe3JUo7ZycKEYQQiQAWd9y54F4XZ\" 0.01 10 \"donation\" \"seans outpost\"") +
            "\nAs a json rpc call\n"
            + HelpExampleRpc("sendfrom", "\"tabby\", \"ST5ZrZNe3JUo7ZycKEYQQiQAWd9y54F4XZ\", 0.01, 10, \"donation\", \"seans outpost\"")
        );

    std::string strAccount = AccountFromValue(params[0]);
    CAltmarketsAddress address(params[1].get_str());
    if (!address.IsValid())
        throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid Altmarkets address");
    CAmount nAmount = AmountFromValue(params[2]);
    if (nAmount < MIN_TXOUT_AMOUNT)
...
UniValue sendmany(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 4)
        throw std::runtime_error(
            ...
            "    {\n"
            "      \"address\":amount   (numeric) The Altmarkets address is the key, the numeric amount in Altmarkets is the value\n"
            "      ,...\n"
            "    }\n"
            ...
            + HelpExampleCli("sendmany", "\"tabby\" \"{\\\"ST5ZrZNe3JUo7ZycKEYQQiQAWd9y54F4XZ\\\":0.01,\\\"ST5ZrZNe3JUo7ZycKEYQQiQAWd9y54F4XZ\\\":0.02}\"") +
            "\nSend two amounts to two different addresses setting the confirmation and comment:\n"
            + HelpExampleCli("sendmany", "\"tabby\" \"{\\\"ST5ZrZNe3JUo7ZycKEYQQiQAWd9y54F4XZ\\\":0.01,\\\"ST5ZrZNe3JUo7ZycKEYQQiQAWd9y54F4XZ\\\":0.02}\" 10 \"testing\"") +
            "\nAs a json rpc call\n"
            + HelpExampleRpc("sendmany", "\"tabby\", \"{\\\"ST5ZrZNe3JUo7ZycKEYQQiQAWd9y54F4XZ\\\":0.01,\\\"ST5ZrZNe3JUo7ZycKEYQQiQAWd9y54F4XZ\\\":0.02}\", 10, \"testing\"")
        );
    ...
    std::set<CAltmarketsAddress> setAddress;
    ...
    {
        CAltmarketsAddress address(name_);
        if (!address.IsValid())
            throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, std::string("Invalid Altmarkets address: ")+name_);
    ...
    int nSplitBlock = 0;
    bool fCreated = pwalletMain->CreateTransaction(vecSend, wtx, keyChange, nFeeRequired, nSplitBlock, strFailReason);
    if (!fCreated)
...
UniValue ListReceived(const UniValue& params, bool fByAccounts)
{
    ...
    // Tally
    std::map<CAltmarketsAddress, tallyitem> mapTally;
    for (std::map<uint256, CWalletTx>::iterator it = pwalletMain->mapWallet.begin(); it != pwalletMain->mapWallet.end(); ++it)
    {
        const CWalletTx& wtx = (*it).second;
    ...
    for(const std::pair<CAltmarketsAddress, CAddressBookData>& item : pwalletMain->mapAddressBook)
    {
        ...
        std::map<CAltmarketsAddress, tallyitem>::iterator it = mapTally.find(address);
...
UniValue listreceivedbyaddress(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() > 3)
        throw std::runtime_error(
            ...
            "    \"amount\" : x.xxx,                  (numeric) The total amount in Altmarkets received by the address\n"
...
UniValue listtransactions(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() > 4)
        throw std::runtime_error(
            ...
            "    \"address\":\"Altmarketsaddress\",    (string) The Altmarkets address of the transaction. Not present for \n"
            "                                                move transactions (category = move).\n"
            "    \"category\":\"send|receive|move\", (string) The transaction category. 'move' is a local (off blockchain)\n"
            "                                                transaction between accounts, and not associated with an address,\n"
            "                                                transaction id or block. 'send' and 'receive' transactions are \n"
            "                                                associated with an address, transaction id and block details\n"
            "    \"amount\": x.xxx,          (numeric) The amount in Altmarkets. This is negative for the 'send' category, and for the\n"
            "                                         'move' category for moves outbound. It is positive for the 'receive' category,\n"
            "                                         and for the 'move' category for inbound funds.\n"
            "    \"vout\" : n,               (numeric) the vout value\n"
            "    \"fee\": x.xxx,             (numeric) The amount of the fee in Altmarkets. This is negative and only available for the \n"
...
UniValue listsinceblock(const UniValue& params, bool fHelp)
{
    if (fHelp)
        throw std::runtime_error(
            ...
            "    \"address\":\"Altmarketsaddress\",    (string) The Altmarkets address of the transaction. Not present for move transactions (category = move).\n"
            "    \"category\":\"send|receive\",     (string) The transaction category. 'send' has negative amounts, 'receive' has positive amounts.\n"
            "    \"amount\": x.xxx,          (numeric) The amount in Altmarkets. This is negative for the 'send' category, and for the 'move' category for moves \n"
            "                                          outbound. It is positive for the 'receive' category, and for the 'move' category for inbound funds.\n"
            "    \"vout\" : n,               (numeric) the vout value\n"
            "    \"fee\": x.xxx,             (numeric) The amount of the fee in Altmarkets. This is negative and only available for the 'send' category of transactions.\n"
...
UniValue gettransaction(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 1 || params.size() > 2)
        throw std::runtime_error(
            ...
            "\nResult:\n"
            "{\n"
            "  \"amount\" : x.xxx,        (numeric) The transaction amount in Altmarkets\n"
            ...
            "      \"address\" : \"Altmarketsaddress\",   (string) The Altmarkets address involved in the transaction\n"
            "      \"category\" : \"send|receive\",    (string) The category, either 'send' or 'receive'\n"
            "      \"amount\" : x.xxx                  (numeric) The amount in Altmarkets\n"
            "      \"vout\" : n,                       (numeric) the vout value\n"
            "    }\n"
            "    ,...\n"
            "  ],\n"
...
UniValue walletpassphrase(const UniValue& params, bool fHelp)
{
    if (pwalletMain->IsCrypted() && (fHelp || params.size() < 2 || params.size() > 3))
        throw std::runtime_error(
            ...
            "This is needed prior to performing transactions related to private keys such as sending Altmarketss\n"
            "\nArguments:\n"
            "1. \"passphrase\"     (string, required) The wallet passphrase\n"
            "2. timeout            (numeric, required) The time to keep the decryption key in seconds.\n"
            "3. mintonly           optional true/false allowing only block staking."
            "\nNote:\n"
...
UniValue encryptwallet(const UniValue& params, bool fHelp)
{
    if (!pwalletMain->IsCrypted() && (fHelp || params.size() != 1))
        throw std::runtime_error(
            ...
            "\nNow set the passphrase to use the wallet, such as for signing or sending Altmarkets\n"
            + HelpExampleCli("walletpassphrase", "\"my pass phrase\"") +
            "\nNow we can so something like sign\n"
            + HelpExampleCli("signmessage", "\"Altmarketsaddress\" \"test message\"") +
...
UniValue listunspent(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() > 3)
        throw std::runtime_error(
            ...
            "3. \"addresses\"    (string) A json array of Altmarkets addresses to filter\n"
            "    [\n"
            "      \"address\"   (string) Altmarkets address\n"
            "      ,...\n"
            "    ]\n"
            "\nResult\n"
            ...
            "    \"vout\" : n,               (numeric) the vout value\n"
            "    \"address\" : \"address\",  (string) the Altmarkets address\n"
            ...
            + HelpExampleCli("listunspent", "10 9999999 \"[\\\"SPGFqEzfmQch1gKD3ra4k18PNj3tTUUSqg\\\",\\\"SIfcnUremQch1gKD3ra4k18PNj3tTUUSqg\\\"]\"")
            + HelpExampleRpc("listunspent", "10, 9999999 \"[\\\"SPGFqEzfmQch1gKD3ra4k18PNj3tTUUSqg\\\",\\\"SIfcnUremQch1gKD3ra4k18PNj3tTUUSqg\\\"]\"")
        );
    ...
    if (params.size() > 2) {
        ...
        for (unsigned int idx = 0; idx < inputs.size(); idx++) {
            ...
            CAltmarketsAddress address(input.get_str());
            if (!address.IsValid())
                throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, std::string("Invalid Altmarkets address: ")+input.get_str());
            if (setAddress.count(address))
    ...
    for(const COutput& out : vecOutputs) {
        ...
        CTxDestination address;
        if (ExtractDestination(out.tx->vout[out.i].scriptPubKey, address)) {
            entry.push_back(Pair("address", CAltmarketsAddress(address).ToString()));
...
UniValue lockunspent(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 1 || params.size() > 2)
        throw std::runtime_error(
            ...
            "A locked transaction output will not be chosen by automatic coin selection, when spending Altmarketss.\n"
...
UniValue settxfee(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 1 || params.size() > 1 || AmountFromValue(params[0]) < MIN_TX_FEE)
        throw std::runtime_error(
            ...
            "\nArguments:\n"
            "1. amount         (numeric, required) The transaction fee in ALTM/kB rounded to the nearest 0.00000001\n"
            "\nResult\n"
...
UniValue getwalletinfo(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 0)
        throw std::runtime_error(
            ...
            "  \"balance\": xxxxxxx,         (numeric) the total Altmarkets balance of the wallet\n"
...
#ifdef ENABLE_WALLET
    { "Decentralised DNS",   "name_scan",              &name_scan,              true,     false,      false },
    { "Decentralised DNS",   "name_filter",            &name_filter,            true,     false,      false },
    { "Decentralised DNS",   "name_show",              &name_show,              true,     false,      false },
    { "Decentralised DNS",   "name_history",           &name_history,           true,     false,      false },
    { "Decentralised DNS",   "name_mempool",           &name_mempool,           true,     false,      false },
    { "Decentralised DNS",   "name_new",               &name_new,               true,     false,      false },
    { "Decentralised DNS",   "name_update",            &name_update,            true,     false,      false },
    { "Decentralised DNS",   "name_delete",            &name_delete,            true,     false,      false },
    { "Decentralised DNS",   "name_list",              &name_list,              true,     false,      false },
#endif // ENABLE_WALLET
