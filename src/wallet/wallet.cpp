...
bool CWallet::LoadCScript(const CScript& redeemScript)
{
    ...
    {
        std::string strAddr = CAltmarketsAddress(CScriptID(redeemScript)).ToString();
    ...
    for (int nRep = 0; nRep < iterations && nBest != nTargetValue; nRep++)
    {
        ...
        for (int nPass = 0; nPass < 2 && !fReachedTarget; nPass++)
        {
            for (unsigned int i = 0; i < vValue.size(); i++)
            {
                ...
                //that the rng is fast. We do not use a constant random Altmarkets,
                                ...
                                CWalletTx& wtxNew, CReserveKey& reservekey, CAmount& nFeeRet, int nSplitBlock, std::string& strFailReason, const CCoinControl* coinControl)
{
    ...
    // Altmarkets: define some values used in case of namecoin tx creation
    ...
    txNew.nVersion = wtxNew.nVersion; // Altmarkets: important for name transactions

    {
        LOCK2(cs_main, cs_wallet);
        {
            nFeeRet = max(nFeeInput, MIN_TX_FEE);  // Altmarkets: a good starting point, probably...
            while (true)
            {
                txNew.vin.clear();
                txNew.vout.clear();
                ...
                // Altmarkets: in case of namecoin tx we have already supplied input.
                ...
                // otherwise proceed as we normaly would in Altmarkets
                else
        ...
        // Altmarkets: name tx always at first position
                if (!wtxNameIn.IsNull())
                {
                ...
                if (nChange > 0)
                {
                    ...
                    // change transaction isn't always pay-to-Altmarkets-address
                ...
                for(const std::pair<const CWalletTx*, unsigned int>& coin : setCoins)
                {
                    // Altmarkets: we sign name tx differently.
...
bool CWallet::SetAddressBook(const CTxDestination& address, const string& strName, const string& strPurpose)
{
    ...
    if (!fFileBacked)
        return false;
    if (!strPurpose.empty() && !CWalletDB(strWalletFile).WritePurpose(CAltmarketsAddress(address).ToString(), strPurpose))
        return false;
    return CWalletDB(strWalletFile).WriteName(CAltmarketsAddress(address).ToString(), strName);
}
...
{
    {
        ...
        if(fFileBacked)
        {
            ...
            std::string strAddress = CAltmarketsAddress(address).ToString();
    ...
    if (!fFileBacked)
        return false;
    CWalletDB(strWalletFile).ErasePurpose(CAltmarketsAddress(address).ToString());
    return CWalletDB(strWalletFile).EraseName(CAltmarketsAddress(address).ToString());
}
...
bool CWallet::AddDestData(const CTxDestination &dest, const std::string &key, const std::string &value)
{
    ...
    if (!fFileBacked)
        return true;
    return CWalletDB(strWalletFile).WriteDestData(CAltmarketsAddress(dest).ToString(), key, value);
}
...
bool CWallet::EraseDestData(const CTxDestination &dest, const std::string &key)
{
    ...
    if (!fFileBacked)
        return true;
    return CWalletDB(strWalletFile).EraseDestData(CAltmarketsAddress(dest).ToString(), key);
}
