...
public:
    ...
    bool GetCoins(const uint256 &txid, CCoins &coins) const {
        try {
            ...
            // Starting the shutdown Altmarkets and returning false to the caller would be
...
void PrepareShutdown()
{
    ...
    RenameThread("Altmarkets-shutoff");
    ...
    ShutdownRPCMining();
#endif
    GenerateAltmarketss(false, NULL, 0, Params());
    StopNode();
	StopTorControl();
...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    strUsage += "  -conf=<file>           " + strprintf(_("Specify configuration file (default: %s)"), "Altmarkets.conf") + "\n";
    if (mode == HMM_AltmarketsD)
    {
#if !defined(WIN32)
...
#ifndef WIN32
    strUsage += "  -pid=<file>            " + strprintf(_("Specify pid file (default: %s)"), "Altmarketsd.pid") + "\n";
#endif
    ...
    if (GetBoolArg("-help-debug", false))
        strUsage += "  -mintxfee=<amt>        " + strprintf(_("Fees (in ALTM/Kb) smaller than this are considered zero fee for transaction creation (default: %s)"), FormatMoney(CWallet::minTxFee.GetFeePerK())) + "\n";
    strUsage += "  -paytxfee=<amt>        " + strprintf(_("Fee (in ALTM/kB) to add to transactions you send (default: %s)"), FormatMoney(payTxFee.GetFeePerK())) + "\n";
    ...
    strUsage += "  -minrelaytxfee=<amt>   " + strprintf(_("Fees (in ALTM/Kb) smaller than this are considered zero fee for relaying (default: %s)"), FormatMoney(::minRelayTxFee.GetFeePerK())) + "\n";
    ...
    strUsage += "\n" + _("RPC SSL options: (see the Altmarkets Wiki for SSL setup instructions)") + "\n";
...
void ThreadImport(std::vector<boost::filesystem::path> vImportFiles)
{
    ...
    RenameThread("Altmarkets-loadblk");
...
/** Sanity checks
 *  Ensure that Altmarkets is running in a usable environment with all
...
bool InitSanityCheck(void)
{
    if(!ECC_InitSanityCheck()) {
        InitError("OpenSSL appears to lack support for elliptic curve cryptography. For more "
                  "information, visit https://en.Altmarkets.it/wiki/OpenSSL_and_EC_Libraries");
        return false;
    }
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. Altmarkets is shutting down."));
...
#endif
    // Make sure only a single Altmarkets process is using the data directory.
    boost::filesystem::path pathLockFile = GetDataDir() / ".lock";
    FILE* file = fopen(pathLockFile.string().c_str(), "a"); // empty lock file; created if it doesn't exist.
    if (file) fclose(file);
    static boost::interprocess::file_lock lock(pathLockFile.string().c_str());
    if (!lock.try_lock())
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. Altmarkets is probably already running."), strDataDir));
#ifndef WIN32
    ...
    if (GetBoolArg("-shrinkdebugfile", !fDebug))
        ShrinkDebugFile();
    LogPrintf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    LogPrintf("Altmarkets version %s\n", FormatFullVersion());
#ifdef ENABLE_WALLET
    ...
    // Altmarkets: check in nameindex need to be created or recreated
        ...
        if (nLoadWalletRet != DB_LOAD_OK)
        {
            if (nLoadWalletRet == DB_CORRUPT)
                strErrors << _("Error loading wallet.dat: Wallet is either corrupted OR you need to use your previous client to dump ALL private keys for ALL addresses."
                               ...
                               " Allow Altmarkets to build a new wallet.dat, fully sync and then import the private keys to the newly created wallet") << "\n";
            else if (nLoadWalletRet == DB_NONCRITICAL_ERROR)
            {
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of Altmarkets") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE)
            {
                strErrors << _("Wallet needed to be rewritten: restart Altmarkets to complete") << "\n";
    ...
    if (pwalletMain)
        GenerateAltmarketss(GetBoolArg("-gen", DEFAULT_GENERATE), pwalletMain, GetArg("-genproclimit", DEFAULT_GENERATE_THREADS), chainparams);
    ...
    {
        #define ALTMDNS_PORT 5335
        int port = GetArg("-seqdnsport", ALTMDNS_PORT);
