...
const QString AltmarketsGUI::DEFAULT_WALLET = "~Default";

AltmarketsGUI::AltmarketsGUI(const NetworkStyle *networkStyle, QWidget *parent) :
    QMainWindow(parent),
    enableWallet(false),
    ...
    spinnerFrame(0)
{
    ...
    QString windowTitle = tr("Altmarkets") + " - ";
#ifdef ENABLE_WALLET
...
void AltmarketsGUI::createActions(const NetworkStyle *networkStyle)
{
    ...
    sendCoinsAction->setStatusTip(tr("Send coins to a Altmarkets address"));
    ...
    receiveCoinsAction->setStatusTip(tr("Request payments (generates QR codes and Altmarkets: URIs)"));
    ...
    dnsAction->setStatusTip(tr("Manage values registered via Altmarkets"));
    ...
    aboutAction = new QAction(networkStyle->getAppIcon(), tr("&About Altmarkets"), this);
    aboutAction->setStatusTip(tr("Show information about Altmarkets"));
    ...
    optionsAction->setStatusTip(tr("Modify configuration options for Altmarkets"));
    ...
    signMessageAction->setStatusTip(tr("Sign messages with your Altmarkets addresses to prove you own them"));
    verifyMessageAction = new QAction(QIcon(":/icons/transaction_0"), tr("&Verify message..."), this);
    verifyMessageAction->setStatusTip(tr("Verify messages to ensure they were signed with specified Altmarkets addresses"));
    ...
    openAction->setStatusTip(tr("Open a Altmarkets: URI or payment request"));

    showHelpMessageAction = new QAction(QApplication::style()->standardIcon(QStyle::SP_MessageBoxInformation), tr("&Command-line options"), this);
    showHelpMessageAction->setMenuRole(QAction::NoRole);
    showHelpMessageAction->setStatusTip(tr("Show the Altmarkets help message to get a list with possible Altmarkets command-line options"));
...
void AltmarketsGUI::createMenuBar()
{
#ifdef Q_OS_MAC
...
void AltmarketsGUI::createToolBars()
{
    if(walletFrame)
    {
...
void AltmarketsGUI::setClientModel(ClientModel *clientModel)
{
...
#ifdef ENABLE_WALLET
bool AltmarketsGUI::addWallet(const QString& name, WalletModel *walletModel)
{
    if(!walletFrame)
        return false;
...
bool AltmarketsGUI::setCurrentWallet(const QString& name)
{
    if(!walletFrame)
        return false;
...
void AltmarketsGUI::removeAllWallets()
{
    if(!walletFrame)
        return;
    setWalletActionsEnabled(false);
...
void AltmarketsGUI::setWalletActionsEnabled(bool enabled)
{
...
void AltmarketsGUI::createTrayIcon(const NetworkStyle *networkStyle)
{
    trayIcon = new QSystemTrayIcon(this);
    QString toolTip = tr("Altmarkets client") + " " + networkStyle->getTitleAddText();
...
void AltmarketsGUI::createTrayIconMenu(QMenu *pmenu)
{
...
#ifndef Q_OS_MAC
void AltmarketsGUI::trayIconActivated(QSystemTrayIcon::ActivationReason reason)
{
...
void AltmarketsGUI::optionsClicked()
{
...
void AltmarketsGUI::aboutClicked()
{
    if(!clientModel)
        return;
...
void AltmarketsGUI::showConfEditor()
{
    GUIUtil::openConfigfile();
}


void AltmarketsGUI::showBackups()
{
    GUIUtil::showBackups();
}

void AltmarketsGUI::showHelpMessageClicked()
{
...
#ifdef ENABLE_WALLET
void AltmarketsGUI::openClicked()
{
...
void AltmarketsGUI::gotoOverviewPage()
{
    overviewAction->setChecked(true);
    if (walletFrame) walletFrame->gotoOverviewPage();
}

void AltmarketsGUI::gotoHistoryPage()
{
    historyAction->setChecked(true);
    if (walletFrame) walletFrame->gotoHistoryPage();
}

void AltmarketsGUI::gotoReceiveCoinsPage()
{
    receiveCoinsAction->setChecked(true);
    if (walletFrame) walletFrame->gotoReceiveCoinsPage();
}

void AltmarketsGUI::gotoSendCoinsPage(QString addr)
{
    sendCoinsAction->setChecked(true);
    if (walletFrame) walletFrame->gotoSendCoinsPage(addr);
}

void AltmarketsGUI::gotoStakeReportPage()
{
    stakeReportAction->setChecked(true);
    if (walletFrame) walletFrame->gotoStakeReportPage();
}

void AltmarketsGUI::gotoDNSPage()
{
    dnsAction->setChecked(true);
    if (walletFrame) walletFrame->gotoDNSPage();
}

void AltmarketsGUI::gotoSignMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoSignMessageTab(addr);
}

void AltmarketsGUI::gotoVerifyMessageTab(QString addr)
{
...
void AltmarketsGUI::setNumConnections(int count)
{
    QString icon;
    switch(count)
    {
    ...
    labelConnectionsIcon->setToolTip(tr("%n active connection(s) to Altmarkets network", "", count));
}

void AltmarketsGUI::updateHeadersSyncProgressLabel()
{
...
void AltmarketsGUI::setNumBlocks(int count, const QDateTime& blockDate, double nVerificationProgress, bool header)
{
    if (modalOverlay)
    {
        if (header)
...
void AltmarketsGUI::message(const QString &title, const QString &message, unsigned int style, bool *ret)
{
    QString strTitle = tr("Altmarkets"); // default title
...
void AltmarketsGUI::changeEvent(QEvent *e)
{
...
void AltmarketsGUI::closeEvent(QCloseEvent *event)
{
#ifndef Q_OS_MAC // Ignored on Mac
    if(clientModel && clientModel->getOptionsModel())
    {
...
#ifdef ENABLE_WALLET
void AltmarketsGUI::incomingTransaction(const QString& date, int unit, const CAmount& amount, const QString& type, const QString& address)
{
    ...
    message((amount)<0 ? tr("Sent transaction") : tr("Incoming transaction"),
             tr("Date: %1\n"
                ...
                "Address: %4\n")
                  .arg(date)
                  .arg(AltmarketsUnits::formatWithUnit(unit, amount, true))
                  .arg(type)
                  .arg(address), CClientUIInterface::MSG_INFORMATION);
}
#endif // ENABLE_WALLET

void AltmarketsGUI::dragEnterEvent(QDragEnterEvent *event)
{
    // Accept only URIs
...
void AltmarketsGUI::dropEvent(QDropEvent *event)
{
    if(event->mimeData()->hasUrls())
    {
...
bool AltmarketsGUI::eventFilter(QObject *object, QEvent *event)
{
...
#ifdef ENABLE_WALLET
bool AltmarketsGUI::handlePaymentRequest(const SendCoinsRecipient& recipient)
{
    // URI has to be valid
...
void AltmarketsGUI::setHDStatus(int hdEnabled)
{
...
void AltmarketsGUI::setEncryptionStatus(int status)
{
    switch(status)
    {
...
void AltmarketsGUI::showNormalIfMinimized(bool fToggleHidden)
{
    if(!clientModel)
        return;
...
void AltmarketsGUI::toggleHidden()
{
...
void AltmarketsGUI::updateWeight()
{
    if (!pwalletMain)
        return;
...
void AltmarketsGUI::updateStakingIcon()
{
    updateWeight();
...
void AltmarketsGUI::detectShutdown()
{
...
void AltmarketsGUI::showProgress(const QString &title, int nProgress)
{
    if (nProgress == 0)
    {
...
void AltmarketsGUI::showModalOverlay()
{
    if (modalOverlay)
        modalOverlay->showHide(false, true);
}

static bool ThreadSafeMessageBox(AltmarketsGUI *gui, const std::string& message, const std::string& caption, unsigned int style)
{
...
void AltmarketsGUI::subscribeToCoreSignals()
{
    // Connect signals to client
    uiInterface.ThreadSafeMessageBox.connect(boost::bind(ThreadSafeMessageBox, this, _1, _2, _3));
}

void AltmarketsGUI::unsubscribeFromCoreSignals()
{
...
void AltmarketsGUI::handleRestart(QStringList args)
{
    if (!ShutdownRequested())
    ...
    menu(0)
{
    ...
    QList<AltmarketsUnits::Unit> units = AltmarketsUnits::availableUnits();
    int max_width = 0;
    const QFontMetrics fm(font());
    Q_FOREACH (const AltmarketsUnits::Unit unit, units)
    {
        max_width = qMax(max_width, fm.width(AltmarketsUnits::name(unit)));
    }
...
{
    menu = new QMenu();
    Q_FOREACH(AltmarketsUnits::Unit u, AltmarketsUnits::availableUnits())
    {
        QAction *menuAction = new QAction(QString(AltmarketsUnits::name(u)), this);
...
{
    setPixmap(QIcon(":/icons/unit_" + AltmarketsUnits::id(newUnits)).pixmap(33,STATUSBAR_ICONSIZE));
}
