...
{
    CAltmarketsAddress addressParsed(address.toStdString());
...
WalletModel::SendCoinsReturn WalletModel::prepareTransaction(WalletModelTransaction &transaction, int nSplitBlock, const CCoinControl *coinControl)
{
    ...
    foreach(const SendCoinsRecipient &rcp, recipients)
    {
        ...
        {   // User-entered Altmarkets address / amount:
            ...
            CScript scriptPubKey = GetScriptForDestination(CAltmarketsAddress(rcp.address.toStdString()).Get());
...

    {
        ...
        // reject insane fee > 0.1 Altmarkets
...
WalletModel::SendCoinsReturn WalletModel::sendCoins(WalletModelTransaction &transaction)
{
...

    {
        ...
        Q_FOREACH(const SendCoinsRecipient &rcp, transaction.getRecipients())
        {
            ...
            else if (!rcp.message.isEmpty()) // Message from normal Altmarkets:URI (Altmarkets:123...?message=example)
...
{
    QString strAddress = QString::fromStdString(CAltmarketsAddress(address).ToString());
...
void WalletModel::listCoins(std::map<QString, std::vector<COutput> >& mapCoins) const
{
    ...
    for(const COutput& out : vCoins)
    {
        ...
        if(!out.fSpendable || !ExtractDestination(cout.tx->vout[cout.i].scriptPubKey, address))
            continue;
        mapCoins[QString::fromStdString(CAltmarketsAddress(address).ToString())].push_back(out);
    }
}
...
{
    CTxDestination dest = CAltmarketsAddress(sAddress).Get();
...
bool WalletModel::isMine(const CAltmarketsAddress &address)
{
