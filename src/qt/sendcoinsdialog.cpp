    ...
    fFeeMinimized(true)
{
    ...
    ui->splitBlockLineEdit->setPlaceholderText(tr("# of Inputs"));
    ui->splitBlockCheckBox->setToolTip(tr("Enable/Disable Input Splitting"));
#endif
...
void SendCoinsDialog::setModel(WalletModel *model)
{
    ...
    if(model && model->getOptionsModel())
    {
        ...
        // Altmarkets: disable fee section
...
void SendCoinsDialog::on_sendButton_clicked()
{
    ...
    {
        SendCoinsEntry *entry = qobject_cast<SendCoinsEntry*>(ui->entries->itemAt(i)->widget());
        CAltmarketsAddress address = entry->getValue().address.toStdString();
    ...
    Q_FOREACH(const SendCoinsRecipient &rcp, recipients)
    {
        ...
        formatted.append(tr("<b>%1</b> to %2 (%3)").arg(AltmarketsUnits::formatWithUnit(AltmarketsUnits::ALTM, rcp.amount), Qt::escape(rcp.label), rcp.address));
#else
        formatted.append(tr("<b>%1</b> to %2 (%3)").arg(AltmarketsUnits::formatWithUnit(AltmarketsUnits::ALTM, rcp.amount), rcp.label.toHtmlEscaped(), rcp.address));
#endif
        } 
        else 
        { 
#if QT_VERSION < 0x050000 
        formatted.append(tr("<b>%1</b> in %4 blocks of %5 ALTM each to %2 (%3)?").arg(AltmarketsUnits::formatWithUnit(AltmarketsUnits::ALTM, rcp.amount),  
            Qt::escape(rcp.label),  
            rcp.address,  
            QString::number(nSplitBlock),  
            AltmarketsUnits::formatWithUnit(AltmarketsUnits::ALTM, rcp.amount / nSplitBlock)));
#else 
        formatted.append(tr("<b>%1</b> in %4 blocks of %5 ALTM each to %2 (%3)?").arg(AltmarketsUnits::formatWithUnit(AltmarketsUnits::ALTM, rcp.amount),  
    ...
    if (model->getOptionsModel()->getCoinControlFeatures()) // coin control enabled
        prepareStatus = model->prepareTransaction(currentTransaction, nSplitBlock, CoinControlDialog::coinControl);
    else
        prepareStatus = model->prepareTransaction(currentTransaction, nSplitBlock);

    // process prepareStatus and on error generate message shown to user
    processSendCoinsReturn(prepareStatus,
        AltmarketsUnits::formatWithUnit(model->getOptionsModel()->getDisplayUnit(), currentTransaction.getTransactionFee()));
    ...
    if(txFee > 0)
    {
        ...
        questionString.append(AltmarketsUnits::formatHtmlWithUnit(model->getOptionsModel()->getDisplayUnit(), txFee));
    ...
    foreach(AltmarketsUnits::Unit u, AltmarketsUnits::availableUnits())
    {
        if(u != model->getOptionsModel()->getDisplayUnit())
            alternativeUnits.append(AltmarketsUnits::formatHtmlWithUnit(u, totalAmount));
    }
    questionString.append(tr("Total Amount %1 (= %2)")
        .arg(AltmarketsUnits::formatHtmlWithUnit(model->getOptionsModel()->getDisplayUnit(), totalAmount))
...
void SendCoinsDialog::setBalance(const CAmount& balance, const CAmount& total, const CAmount &stake, const CAmount& unconfirmedBalance, const CAmount& immatureBalance,
                                 const CAmount& watchBalance, const CAmount& watchStake, const CAmount& watchUnconfirmedBalance, const CAmount& watchImmatureBalance)
{
    ...
    if(model && model->getOptionsModel())
    {
        ui->labelBalance->setText(AltmarketsUnits::formatWithUnit(model->getOptionsModel()->getDisplayUnit(), balance));
    }
}
...
void SendCoinsDialog::processSendCoinsReturn(const WalletModel::SendCoinsReturn &sendCoinsReturn, const QString &msgArg)
{
    ...
    case WalletModel::InsaneFee:
        msgParams.first = tr("A fee higher than %1 is considered an insanely high fee.").arg(AltmarketsUnits::formatWithUnit(model->getOptionsModel()->getDisplayUnit(), 10000000));
        break;
...
void SendCoinsDialog::updateFeeMinimizedLabel()
{
    ...
    if (ui->radioSmartFee->isChecked())
        ...
        ui->labelFeeMinimized->setText(AltmarketsUnits::formatWithUnit(model->getOptionsModel()->getDisplayUnit(), ui->customFee->value()) +
...
void SendCoinsDialog::updateMinFeeLabel()
{
    if (model && model->getOptionsModel())
        ui->checkBoxMinimumFee->setText(tr("Pay only the minimum fee of %1").arg(
            AltmarketsUnits::formatWithUnit(model->getOptionsModel()->getDisplayUnit(), CWallet::minTxFee.GetFeePerK()) + "/kB")
        );
}
...
void SendCoinsDialog::updateSmartFeeLabel()
{
    ...
    {
        ui->labelSmartFee->setText(AltmarketsUnits::formatWithUnit(model->getOptionsModel()->getDisplayUnit(), CWallet::minTxFee.GetFeePerK()) + "/kB");
        ui->labelSmartFee2->show(); // (Smart fee not initialized yet. This usually takes a few blocks...)
        ui->labelFeeEstimation->setText("");
    }
    else
    {
        ui->labelSmartFee->setText(AltmarketsUnits::formatWithUnit(model->getOptionsModel()->getDisplayUnit(), feeRate.GetFeePerK()) + "/kB");
...
void SendCoinsDialog::coinControlChangeEdited(const QString& text)
{
    if (model && model->getAddressTableModel())
    {
        ...
        CAltmarketsAddress addr = CAltmarketsAddress(text.toStdString());
        ...
        {
            ui->labelCoinControlChangeLabel->setText(tr("Warning: Invalid Altmarkets address"));
        }
        else // Valid address
        {
            CPubKey pubkey;
