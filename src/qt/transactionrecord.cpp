...
QList<TransactionRecord> TransactionRecord::decomposeTransaction(const CWallet *wallet, const CWalletTx &wtx)
{
    ...
    if (wtx.nVersion == NAMECOIN_TX_VERSION) // Altmarkets: name transaction
    {
    ...
    else if (wtx.IsCoinStake()) // ppcoin: coinstake transaction
    {
        ...
        {
            txrCoinStake.address = CAltmarketsAddress(address).ToString();
        }
    ...
    else if (nNet > 0 || wtx.IsCoinBase())
    {
        ...
        for(const CTxOut& txout : wtx.vout)
        {
            ...
            if(mine)
            {
                ...
                CTxDestination address;
                sub.idx = parts.size(); // Altmarkets number
                sub.credit = txout.nValue;
                sub.involvesWatchAddress = mine == ISMINE_WATCH_ONLY;
                if (ExtractDestination(txout.scriptPubKey, address) && IsMine(*wallet, address))
                {
                    // Received by Altmarkets Address
                    sub.type = TransactionRecord::RecvWithAddress;
                    sub.address = CAltmarketsAddress(address).ToString();
                }
                else
                {
