...
// format Altmarketsvalue with all trailing zero
QString Coin_0Pad(int nUnit, int64_t amount)
{
    QString result = AltmarketsUnits::format(nUnit, amount);

    int poin = result.indexOf(".") + 1;
    poin += AltmarketsUnits::decimals(nUnit);
...
void StakeReportDialog::updateStakeReport(bool fImmediate=false)
{
    ...
    ui->L_Coin->setText(AltmarketsUnits::name(nDisplayUnit) + " " + tr("SubTotal"));

    QTableWidget *TableW = ui->StakeReportTable;

    TableW->horizontalHeaderItem(1)->setText(AltmarketsUnits::name(nDisplayUnit) + " " +tr("Amount"));
    ...
    ui->Amount_24H->setText(Coin_0Pad(nDisplayUnit, aRange[i].Total) + tr(" ALTM"));
    ui->Stake_24H->setText(QString::number(aRange[i++].Count));
    ui->Amount_7D->setText(Coin_0Pad(nDisplayUnit, aRange[i].Total) + tr(" ALTM"));
    ui->Stake_7D->setText(QString::number(aRange[i++].Count));
    ui->Amount_30D->setText(Coin_0Pad(nDisplayUnit, aRange[i].Total) + tr(" ALTM"));
    ui->Stake_30D->setText(QString::number(aRange[i++].Count));
    ui->Amount_365D->setText(Coin_0Pad(nDisplayUnit, aRange[i].Total) + tr(" ALTM"));
    ...
    if (IsInitialBlockDownload())
    {
        ...
        ui->Amount_Last->setText(tr("Amount: ") + Coin_0Pad(nDisplayUnit, aRange[i].Total) + tr(" ALTM"));
...
void StakeReportDialog::CopyAllToClipboard()
{
    ...
    Repo += "           Altmarkets Stake Mini Report\n";
    Repo += "           ----------------------\n";
