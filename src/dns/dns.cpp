...
bool GetNameCurrentAddress(const CNameVal& name, CAltmarketsAddress& address, string& error)
{
    CNameDB dbName("r");
...
NameTxReturn name_operation(const int op, const CNameVal& name, CNameVal value, const int nRentalDays, const string& strAddress, const string& strValueType)
{
    ...
    if (IsInitialBlockDownload())
    {
        ...
        ret.err_msg = "Altmarkets is downloading blocks...";
        return ret;
    }
...

    {
        ...
        {
            CAltmarketsAddress address(strAddress);
            if (!address.IsValid())
            {
                ret.err_code = RPC_INVALID_ADDRESS_OR_KEY;
                ret.err_msg = "Altmarkets address is invalid";
                return ret;
            }
    ...
    {
        ret.address = CAltmarketsAddress(address).ToString();
    }
    ...
    for (unsigned int i = 0; i < tx.vout.size(); i++)
    {
        ...
        if (DecodeNameScript(out.scriptPubKey, ntiTmp, pc))
        {
            ...
            if (checkAddressAndIfIsMine)
            {
                ...
                if (!ExtractDestination(scriptPubKey, address))
                    nti.strAddress = "";
                nti.strAddress = CAltmarketsAddress(address).ToString();
