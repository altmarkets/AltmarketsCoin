...
/*
 * Simple DNS server for Altmarkets project
 *
...
SeqDns::SeqDns(const char *bind_ip, uint16_t port_no,
   const char *gw_suffix, const char *allowed_suff, const char *local_fname, const char *enums, const char *tollfree, uint8_t verbose) 
    ...
    if(ret < 0) {
      throw runtime_error("SeqDns::SeqDns: Cannot create socket");
    } else {
      m_sockfd = ret;
    }
    ...
    if(::bind(m_sockfd, (struct sockaddr *) &m_address, sizeof (struct sockaddr_in)) < 0) {
      char buf[80];
      sprintf(buf, "SeqDns::SeqDns: Cannot bind to port %u", port_no);
    ...
    m_dap_ht  = (allowed_len && m_gw_suf_len)? (DNSAP*)calloc(ALTMDNS_DAPSIZE, sizeof(DNSAP)) : NULL; 
   ...
   *p = 64;
     ...
     if(c == '$') {
       ...
       char *pp = p; // ref to $
	    ...
	    m_allowed_qty++;
     if(m_verbose > 3)
       LogPrintf("\tSeqDns::EmcDns: Insert %s TLD=%s:%u\n", dnstype, p + 1, *p);
	  }
   pos = step = 0;
	  continue;
	}
	...
	step |= 1;
	if(m_verbose > 3)
	  LogPrintf("\tSeqDns::SeqDns: Insert Local:[%s]->[%s] pos=%u step=%u\n", p, p_eq, pos, step);
	do 
	  pos += step;
        while(m_ht_offset[pos] != 0);
    ...
    if(m_verbose > 0)
	 LogPrintf("SeqDns::SeqDns: Created/Attached: %s:%u; Qty=%u:%u\n", 
    ...
    if(tollfree && *tollfree) {
      if(m_verbose > 3)
    LogPrintf("\tSeqDns::EmcDns: Setup deferred toll-free=%s\n", tollfree);
...
SeqDns::~SeqDns() {
    ...
    free(m_dap_ht);
    if(m_verbose > 0)
	 LogPrintf("SeqDns::~SeqDns: Destroyed OK\n");
} // SeqDns::~SeqDns


/*---------------------------------------------------*/

void SeqDns::StatRun(void *p) {
  SeqDns *obj = (SeqDns*)p;
  obj->Run();
//Altmarkets  ExitThread(0);
} // SeqDns::StatRun

/*---------------------------------------------------*/
void SeqDns::Run() {
  if(m_verbose > 2) LogPrintf("SeqDns::Run: started\n");
  ...
  if(m_verbose > 2) LogPrintf("SeqDns::Run: Received Exit packet_len=%d\n", m_rcvlen);

} //  SeqDns::Run

/*---------------------------------------------------*/

void SeqDns::HandlePacket() {
  if(m_verbose > 2) LogPrintf("SeqDns::HandlePacket: Handle packet_len=%d\n", m_rcvlen);
  ...
  if(m_verbose > 3) {
    LogPrintf("\tSeqDns::HandlePacket: msgID  : %d\n", m_hdr->msgID);
    LogPrintf("\tSeqDns::HandlePacket: Bits   : %04x\n", m_hdr->Bits);
    LogPrintf("\tSeqDns::HandlePacket: QDCount: %d\n", m_hdr->QDCount);
    LogPrintf("\tSeqDns::HandlePacket: ANCount: %d\n", m_hdr->ANCount);
    LogPrintf("\tSeqDns::HandlePacket: NSCount: %d\n", m_hdr->NSCount);
    LogPrintf("\tSeqDns::HandlePacket: ARCount: %d\n", m_hdr->ARCount);
  }
...
//*  uint16_t zCount = m_hdr->ANCount | m_hdr->NSCount | m_hdr->ARCount | (m_hdr->Bits & (m_hdr->QR_MASK | m_hdr->TC_MASK));
 ...
 // Fill deferred toll-free default entries
        ...
        while(char *tf_fname = strsep(&tf_str, "|")) {
          if(m_verbose > 3)
      LogPrintf("\tSeqDns::HandlePacket: handle deferred toll-free=%s\n", tf_fname);
    ...
    for(uint16_t qno = 0; qno < m_hdr->QDCount && m_snd < m_bufend; qno++) {
      if(m_verbose > 5) 
        LogPrintf("\tSeqDns::HandlePacket: qno=%u m_hdr->QDCount=%u\n", qno, m_hdr->QDCount);
...
uint16_t SeqDns::HandleQuery() {
  ...
  *--key_end = 0; // Remove last dot, set EOLN
  ...
  if(m_verbose > 3) 
    LogPrintf("SeqDns::HandleQuery: After TLD-suffix cut: [%s]\n", key);
  ...
  if(p) {
    ...
    if(m_allowed_qty) { // Activated TLD-filter
      if(*p != '.') {
        if(m_verbose > 3) 
      LogPrintf("SeqDns::HandleQuery: TLD-suffix=[.%s] is not specified in given key=%s; return NXDOMAIN\n", p, key);
	return 3; // TLD-suffix is not specified, so NXDOMAIN
      ...
      do {
        ...
        if(m_ht_offset[pos] == 0) {
          if(m_verbose > 3) 
  	    LogPrintf("SeqDns::HandleQuery: TLD-suffix=[.%s] in given key=%s is not allowed; return NXDOMAIN\n", p, key);
...
int SeqDns::TryMakeref(uint16_t label_ref) {
  ...
  m_hdr->ANCount = 0;
  LogPrintf("SeqDns::TryMakeref: Generated REF NS=%u\n", m_hdr->NSCount);
  return m_hdr->NSCount;
} //  SeqDns::TryMakeref
/*---------------------------------------------------*/

int SeqDns::Tokenize(const char *key, const char *sep2, char **tokens, char *buf) {
  int tokensN = 0;
...
void SeqDns::Answer_ALL(uint16_t qtype, char *buf) {
  const char *key;
  switch(qtype) {
      case  1 : key = "A";      break;
      case  2 : key = "NS";     break;
  ...
  if(m_verbose > 0) LogPrintf("SeqDns::Answer_ALL(QT=%d, key=%s); TokenQty=%d\n", qtype, key, tokQty);
  ...
  for(int tok_no = 0; tok_no < tokQty; tok_no++) {
      if(m_verbose > 1) 
	LogPrintf("\tSeqDns::Answer_ALL: Token:%u=[%s]\n", tok_no, tokens[tok_no]);
      Out2(m_label_ref);
...
void SeqDns::Fill_RD_IP(char *ipddrtxt, int af) {
  uint16_t out_sz;
  switch(af) {
...
void SeqDns::Fill_RD_DName(char *txt, uint8_t mxsz, int8_t txtcor) {
  uint8_t *snd0 = m_snd;
...
} // SeqDns::Fill_RD_DName

/*---------------------------------------------------*/
/*---------------------------------------------------*/

int SeqDns::Search(uint8_t *key) {
  if(m_verbose > 1) 
    LogPrintf("SeqDns::Search(%s)\n", key);
...
int SeqDns::LocalSearch(const uint8_t *key, uint8_t pos, uint8_t step) {
  if(m_verbose > 1) 
    LogPrintf("SeqDns::LocalSearch(%s, %u, %u) called\n", key, pos, step);
    do {
      pos += step;
      if(m_ht_offset[pos] == 0) {
        if(m_verbose > 3) 
  	  LogPrintf("SeqDns::LocalSearch: Local key=[%s] not found; go to nameindex search\n", key);
...
DNSAP *SeqDns::CheckDAP(uint32_t ip_addr) { 
  uint32_t hash = ip_addr * m_daprand;
  hash ^= hash >> 16;
  hash += hash >> 8;
  DNSAP *dap = m_dap_ht + (hash & (ALTMDNS_DAPSIZE - 1));
  ...
  return (dap->ed_size <= ALTMDNS_DAPTRESHOLD)? dap : NULL;
} // SeqDns::CheckDAP 
...
int SeqDns::SpfunENUM(uint8_t len, uint8_t **domain_start, uint8_t **domain_end) {
  int dom_length = domain_end - domain_start;
  const char *tld = (const char*)domain_end[-1];

  if(m_verbose > 3)
    LogPrintf("\tSeqDns::SpfunENUM: Domain=[%s] N=%u TLD=[%s] Len=%u\n", 
  ...
  *pitut++ = *p;
    ...
    *pitut = 0; // EOLN at phone number end
    ...
    if(m_verbose > 3)
      LogPrintf("\tSeqDns::SpfunENUM: ITU-T num=[%s]\n", itut_num);
    ...
    if(!m_verifiers.empty())
      for(int16_t qno = 0; qno >= 0; qno++) {
        ...
        if(m_verbose > 1) 
          LogPrintf("\tSeqDns::SpfunENUM Search(%s)\n", q_str);
...
void SeqDns::Answer_ENUM(const char *q_str) {
...
bool SeqDns::CheckEnumSig(const char *q_str, char *sig_str) {
  ...
  CAltmarketsAddress addr(nti.strAddress);
        if(!addr.IsValid())
