...
#if defined(NDEBUG)
# error "Altmarkets cannot be compiled without assertions."
#endif
...
CHooks* hooks = InitHook(); //this adds namecoin hooks which allow splicing of code inside standart Altmarkets functions.
...
namespace {
    ...
    struct CBlockIndexWorkComparator
    {
        bool operator()(CBlockIndex *pa, CBlockIndex *pb) const {
            ...
            if (pa->nAltmarketsId < pb->nAltmarketsId) return false;
            if (pa->nAltmarketsId > pb->nAltmarketsId) return true;
     ...
     */
    CCriticalSection cs_nBlockAltmarketsId;
    /** Blocks loaded from disk are assigned id 0, so start the counter at 1. */
    uint32_t nBlockAltmarketsId = 1;
...
void MarkBlockAsInFlight(NodeId nodeid, const uint256& hash, const Consensus::Params& consensusParams, CBlockIndex *pindex = NULL) {
    ...
    QueuedBlock newentry = {hash, pindex, false, pindex != NULL};
...
CAmount GetProofOfWorkReward()
{
    if (chainActive.Height() == 0) {
        CAmount nSubsidy = 44000000 * COIN; // 44,000,000 for Altmarkets Swap
...
void CheckForkWarningConditions()
{
    ...
    if (pindexBestForkTip || (pindexBestInvalid && pindexBestInvalid->nChainWork > chainActive.Tip()->nChainWork + (GetBlockTrust(*chainActive.Tip()) * 6)))
    {
...
void CheckForkWarningConditionsOnNewFork(CBlockIndex* pindexNewForkTip)
{
    ...
    if (pfork && (!pindexBestForkTip || (pindexBestForkTip && pindexNewForkTip->nHeight > pindexBestForkTip->nHeight)) &&
            pindexNewForkTip->nChainWork - pfork->nChainWork > (GetBlockTrust(*pfork) * 7) &&
...
bool CheckInputs(const CTransaction& tx, CValidationState &state, const CCoinsViewCache &inputs, bool fScriptChecks, unsigned int flags, bool cacheStore, std::vector<CScriptCheck> *pvChecks)
{
    if (!tx.IsCoinBase())
    {
        ...
        if (tx.IsCoinStake())
        {
            ...
            if (nStakeReward > GetProofOfStakeReward() - tx.GetMinFee() + MIN_TX_FEE)
...
bool DisconnectBlock(CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool* pfClean, bool fWriteNames)
{
    ...
    // Altmarkets: undo name transactions in reverse order
    if (fWriteNames)
        for (int i = block.vtx.size() - 1; i >= 0; i--)
...
void ThreadScriptCheck() {
    RenameThread("Altmarkets-scriptch");
...
bool ConnectBlock(const CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool fJustCheck, bool fWriteNames)
{
    ...
    // Start enforcing CHECKLOCKTIMEVERIFY, (BIP65) for block.nVersion=2
    // blocks, when 75% of the network has upgraded:
    if (block.nVersion >= 2 && IsSuperMajority(2, pindex->pprev, consensusParams.nEnforceBlockUpgradeMajority, consensusParams)) {
    ...
    // Altmarkets: collect valid name tx
    ...
    // Altmarkets: add names to nameindex.dat
    if (fWriteNames)
...
void static UpdateTip(CBlockIndex *pindexNew) {
    ...
    LogPrintf("UpdateTip: new best=%s  height=%d  log2_trust=%.8g  tx=%lu  date=%s progress=%f  cache=%.1fMiB(%utx)  moneysupply=%s\n",
 ...
 */
bool ActivateBestChain(CValidationState &state, const CChainParams& chainparams, CBlock *pblock, CBlockIndex *pindexSwitchTo) {
...
bool GetCoinAge(const CTransaction& tx, CValidationState &state, const CCoinsViewCache &view, uint64_t& nCoinAge)
{
    ...
    for(const CTxIn& txin : tx.vin)
    {
        ...
        if (pblocktree->ReadTxIndex(prevout.hash, postx))
        {
            ...
            const Consensus::Params& consensusParams = Params().GetConsensus();
            if (header.GetBlockTime() + consensusParams.nStakeMinAge > tx.nTime)
                continue;
...
CBlockIndex* AddToBlockIndex(const CBlockHeader& block)
{
    ...
    assert(pindexNew);
    // We assign the Altmarkets id to blocks only when the full data is available,
    // to avoid miners withholding blocks but broadcasting headers, to get a
    // competitive advantage.
    pindexNew->nAltmarketsId = 0;
...
bool ReceivedBlockTransactions(const CBlock &block, CValidationState& state, CBlockIndex *pindexNew, const CDiskBlockPos& pos)
{
    ...
    if (pindexNew->pprev == NULL || pindexNew->pprev->nChainTx) {
        ...
        while (!queue.empty()) {
            ...
            {
                LOCK(cs_nBlockAltmarketsId);
                pindex->nAltmarketsId = nBlockAltmarketsId++;
            }
...
bool CheckBlock(const CBlock& block, CValidationState& state, bool fCheckPOW, bool fCheckMerkleRoot, bool fCheckSign)
{
    ...
    if (fCheckMerkleRoot) {
        ...
        // Check for merkle tree malleability (CVE-2012-2459): repeating Altmarketss
...
bool ContextualCheckBlockHeader(const CBlockHeader& block, bool fProofOfStake, CValidationState& state, CBlockIndex * const pindexPrev)
{
    ...
    if (block.nVersion < 2 && IsSuperMajority(2, pindexPrev, consensusParams.nRejectBlockOutdatedMajority, consensusParams))
    {
...
bool ContextualCheckBlock(const CBlock& block, CValidationState& state, CBlockIndex * const pindexPrev)
{
    ...
    if (block.nVersion >= 2 && IsSuperMajority(2, pindexPrev, consensusParams.nEnforceBlockUpgradeMajority, consensusParams))
    {
...
bool AcceptBlockHeader(const CBlockHeader& block, bool fProofOfStake, CValidationState& state, const CChainParams& chainparams, CBlockIndex** ppindex)
{
    AssertLockHeld(cs_main);
    // Check for duplicate
    uint256 hash = block.GetHash();
    ...
    // Altmarkets: set PoS flag for CBlockIndex. This should probably be done immediately after we added header to CBlockIndex (i.e., after or inside AddToBlockIndex),
...
bool static LoadBlockIndexDB()
{
    ...
    {
        CBlockIndex* pindex = item.second;
        pindex->nChainWork = (pindex->pprev ? pindex->pprev->nChainWork : 0) + GetBlockTrust(*pindex);
        if (pindex->nStatus & BLOCK_HAVE_DATA) {
            if (pindex->pprev) {
    ...
    LogPrintf("LoadBlockIndexDB(): hashBestChain=%s height=%d date=%s progress=%f trust=%s\n",
        ...
        DateTimeStrFormat("%Y-%m-%d %H:%M:%S", chainActive.Tip()->GetBlockTime()),
        Checkpoints::GuessVerificationProgress(chainParams.Checkpoints(), chainActive.Tip()), chainActive.Tip()->nChainWork.ToString());
...
void static CheckBlockIndex(const Consensus::Params& consensusParams)
{
    ...
    while (pindex != NULL) {
        ...
        if (pindex->nChainTx == 0) assert(pindex->nAltmarketsId == 0);  // nAltmarketsId can't be set for blocks that aren't linked
...
string GetWarnings(string strFor)
{
    ...
    // ppcoin: wallet lock warning for staking
    if (strMintWarning != "")
    {
        nPriority = 0;
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv, int64_t nTimeReceived)
{
    ...
    // Altmarkets: set/unset network serialization mode for new clients
    ...
    else if (strCommand == "getblocks")
    {
        ...
        for (; pindex; pindex = chainActive.Next(pindex))
        {
            if (pindex->GetBlockHash() == hashStop)
            {
                ...
                const Consensus::Params& consensusParams = Params().GetConsensus();
                if (hashStop != chainActive.Tip()->GetBlockHash() && pindex->GetBlockTime() + consensusParams.nStakeMinAge > chainActive.Tip()->GetBlockTime())
    ...
    else if (strCommand == "headers" && !fImporting && !fReindex) // Ignore headers received while importing
    {
        ...
        for(const CBlockHeader& header : headers) {
            ...
            if (pindexLast != NULL && header.hashPrevBlock != pindexLast->GetBlockHash()) {
                Misbehaving(pfrom->GetId(), 20);
                return error("non-continuous headers Altmarkets");
            }
        if (!AcceptBlockHeader(header, header.nFlags & BLOCK_PROOF_OF_STAKE, state, chainparams, &pindexLast)) {                
                int nDoS;
                if (state.IsInvalid(nDoS)) {
                    if (nDoS > 0)
                        Misbehaving(pfrom->GetId(), nDoS);
