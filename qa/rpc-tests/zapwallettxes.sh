...
if [ $# -lt 1 ]; then
        ...
        echo "e.g. $0 ../../src"
        echo "Env vars AltmarketsD and AltmarketsCLI may be used to specify the exact binaries used"
        exit 1
fi

set -f

AltmarketsD=${AltmarketsD:-${1}/Altmarketsd}
CLI=${AltmarketsCLI:-${1}/Altmarkets-cli}
...
$AltmarketsD -zapwallettxes=1 $B1ARGS &
B1PID=$!
$AltmarketsD -zapwallettxes=2 $B2ARGS &
B2PID=$!
